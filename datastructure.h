...
public:
  ...
  myVertex2D(const Point &p, Face_handle cell) : Vb(p, cell) { init(); }
...
public:
  ...
  myFace2D(Vertex_handle v0, Vertex_handle v1, Vertex_handle v2, Face_handle n0,
...
public:
  ...
  myVertex3D(const Point &p, Cell_handle cell) : Vb(p, cell) { init(); }
  myVertex3D(Cell_handle cell) : Vb(cell) { init(); }

  // if the vertex from the original surface, origin_id is its index in the
  // original surface
  // otherwise -1;
  int origin_id;
  unsigned char flag;
...
#define FACET_FLAG_DEEPINT                                                     \
  0x2 // the cell deeply intersects with the adjacent cell along the ith facet
...
#define CELL_FLAG_INVALID_CC                                                   \
  0x04 // the circumcenter of the cell can not be calculated properly
#define CELL_FLAG_HAVING_CC                                                    \
  0x08 // the circumcenter of the cell has been calculated
...
#define CELL_FLAG_TINSIDE                                                      \
  0x10 // temporarily mark the cell which is inside the given mesh
#define CELL_FLAG_TOUTSIDE                                                     \
  0x20 // temporarily mark the cell which is inside the given mesh
...
public:
  ...
  myCell3D() { init(); }
  myCell3D(Vertex_handle v0, Vertex_handle v1, Vertex_handle v2,
           Vertex_handle v3)
      : Cb(v0, v1, v2, v3) {
    init();
  }
  myCell3D(Vertex_handle v0, Vertex_handle v1, Vertex_handle v2,
           Vertex_handle v3, Cell_handle n0, Cell_handle n1, Cell_handle n2,
           Cell_handle n3)
  ...
  bool check_ith_facet_flag(int i, unsigned char f) {
    return facet_flag[i] & f;
  }
  ...
  //  return CGAL::to_double((voronoi_vertex - vertex(0)->point()) *
  //  (voronoi_vertex - vertex(0)->point()) );
  //  }
};
...
enum pmode { EIGEN, FUNCTION };
enum mmode { GRAPH, PCLOUD };

enum nmode { DENSE, SPARSE };

enum cmode { MAT, CXX };

enum stype { SYM, NONSYM, SPHERE, PLANE, SKINSURF };
