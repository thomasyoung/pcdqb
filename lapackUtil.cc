...
int convertIJS2colMatrix(std::vector<unsigned int> II,
                         std::vector<unsigned int> JJ, std::vector<double> SS,
                         double *a, int m, int n) {
  if ((m * n <= 0) || (II.size() != JJ.size()) || (JJ.size() != SS.size()) ||
      (a == NULL))
    return -1;
...
int readPurePointsFile(const char *filename,
...
bool save_IJS(const char *filename, std::vector<unsigned int> &II,
              std::vector<unsigned int> &JJ, std::vector<double> &SS) {

  if (filename == NULL)
    return false;
  if ((II.size() < SS.size()) || (JJ.size() < SS.size())) {
    cerr << "save_IJS: size of parameters do not match: " << II.size()
         << JJ.size() << SS.size() << endl;
    return false;
  }
 ...
 * intends to encapsulate lapack routines for eigen-solving:
 ...
 GEBAK    transform eigenvectors of balanced matrix to those of the original
 matrix
...
extern "C" {
/* Subroutine */ int dgebal_(char *job, long int *n, double *a, long int *lda,
                             long int *ilo, long int *ihi, double *scale,
                             long int *info);
/* Subroutine */ int dgehrd_(long int *n, long int *ilo, long int *ihi,
                             double *a, long int *lda, double *tau,
...
/* Subroutine */ int dhseqr_(char *job, char *compz, long int *n, long int *ilo,
                             long int *ihi, double *h__, long int *ldh,
                             double *wr, double *wi, double *z__, long int *ldz,
                             double *work, long int *lwork, long int *info);
/* Subroutine */ int dtrevc_(char *side, char *howmny, long int *select,
                             long int *n, double *t, long int *ldt, double *vl,
                             long int *ldvl, double *vr, long int *ldvr,
                             long int *mm, long int *m, double *work,
                             long int *info);
/* Subroutine */ int dgebak_(char *job, char *side, long int *n, long int *ilo,
                             long int *ihi, double *scale, long int *m,
                             double *v, long int *ldv, long int *info);

void dsygst_(long int *itype, char *uplo, long int *n, double *a, long int *lda,
             double *b, long int *ldb, long int *info);
/*
...
extern "C" {
void dgees_(char *jobvs, char *sort, int *select, int *n, double *a, int *lda,
            int *sdim, double *wr, double *wi, double *vs, int *ldvs,
            double *work, int *lwork, int *bwork, int *info);
// a simple driver that computes all or part of the Schur factorization of A,
// with optional ordering of the eigenvalues;
int dgeev_(char *jobvl, char *jobvr, long int *n, double *a, long int *lda,
           double *wr, double *wi, double *vl, long int *ldvl, double *vr,
           long int *ldvr, double *work, long int *lwork, long int *info);
// a simple driver that computes all the eigenvalues of A, and (optionally) the
 ...
 *
 */
int solve_eigen(double *v, double *lambda, long int n) {
  ...
  /*
   *    int dgeev_(char *jobvl, char *jobvr, long int *n, double *a, long int
   *lda, double *wr, double *wi, double *vl,
  ...
  dgeev_(&jobvl, &jobvr, &n, v, &n, wr, wi, vl, &ldvl, vr, &ldvr, work, &lwork,
         &info);
  if (0 != info) {
...
int convert_IJS_2_col_sq_matrix(double *result, std::vector<unsigned int> II,
